        ...
        if($request->request_btn!=null && (Auth::user()->getSec()->product_adjustment_request_value || Auth::user()->getSec()->global_admin_value)){
            ...
            if($request->approval_request=='false'){
                ...
                foreach($approval_users as $user){
                    ...
                    if($user['contact_number'] != '') Functions::sms_($user['contact_number'], '['.date("Y-m-d\TH:i").'] Product Adjustment Requested on Product '.$product_description.' by '.Auth::user()->name.' '.Auth::user()->last_name.'. Please review at your earliest convenience at '.env('APP_URL','').'/products', '', '');
